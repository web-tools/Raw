...
((($, undefined) => {
    ...
    var MEASURE_SCROLLBAR_TEMPLATE = "<div class='select2-measure-scrollbar'></div>";
    ...
    nextUid=((() => { var counter=1; return () => counter++; })());
    ...
    function installDebouncedScroll(threshold, element) {
        var notify = debounce(threshold, e => { element.trigger("scroll-debounced", e);});
        element.on("scroll", e => {
    ...
    function ajax(options) {
        var // current scheduled but not yet executed request
        timeout;
        ...
        return query => {
            window.clearTimeout(timeout);
            timeout = window.setTimeout(() => {
                var // ajax data function
                data = options.data;

                var // ajax url string or function
                url = ajaxUrl;

                var transport = options.transport || $.fn.select2.ajaxDefaults.transport;

                var // deprecated - to be removed in 4.0  - use params instead
                deprecated = {
                ...
                var params = $.extend({}, $.fn.select2.ajaxDefaults.params, deprecated);
    ...
    function local(options) {
        var // data elements
        data = options; // function used to retrieve the text portion of a data item that is matched against the search
    ...
    function tags(data) {
        ...
        return query => {
            ...
            $(isFunc ? data() : data).each(function () {
                ...
                var text = isObject ? this.text : this;
    ...
    function defaultTokenizer(input, selection, selectCallback, opts) {
        var // store the original so we can compare and know if we need to tell the search to update its text
        original = input; // the matched separator
        ...
        var // position at which the separator was found
        index;

        var i;

        var // looping variables
        l;
    ...
    AbstractSelect2 = clazz(Object, {
        ...
        init(opts) {
            ...
            this.body = thunk(() => opts.element.closest("body"));
            ...
            this.dropdown.on("click mouseup mousedown", e => { e.stopPropagation(); });
        ...
        prepareOpts(opts) {
            ...
            opts = $.extend({}, {
                populateResults(container, results, query) {
                    var populate;
                    var id=this.opts.id;
                    var liveRegion=this.liveRegion;
            ...
            if (opts.createSearchChoicePosition === 'top') {
                opts.createSearchChoicePosition = (list, item) => { list.unshift(item); };
            }
            else if (opts.createSearchChoicePosition === 'bottom') {
                opts.createSearchChoicePosition = (list, item) => { list.push(item); };
            }
        ...
        positionDropdown() {
            ...
            var width = this.container.outerWidth(false);
            var dropHeight = $dropdown.outerHeight(false);
            var $window = $(window);
            var windowWidth = $window.width();
            var windowHeight = $window.height();
            var viewPortRight = $window.scrollLeft() + windowWidth;
            ...
            var enoughRoomAbove = (offset.top - dropHeight) >= this.body().scrollTop();
            var dropWidth = $dropdown.outerWidth(false);
            var enoughRoomOnRight = dropLeft + dropWidth <= viewPortRight;
            var aboveNow = $dropdown.hasClass("select2-drop-above");
            var bodyOffset;
            var above;
            var changeDirection;
            var css;
        ...
        loadMoreIfNeeded() {
            var results = this.results;
            var more = results.find("li.select2-more-results");

            var // pixels the element is below the scroll fold, below==0 is when the element is starting to be visible
            below;

            var page = this.resultsPage + 1;
            var self=this;
        ...
        updateResults(initial) {
            ...
            var input;
            var term = search.val();
            var lastTerm = $.data(this.container, "select2-last-term");

            var // sequence number used to drop out-of-order responses
            queryNumber;
        ...
        selectHighlighted(options) {
            var index=this.highlight();
            var highlighted=this.results.find(".select2-highlighted");
            var data = highlighted.closest('.select2-result').data("select2-data");
    ...
    SingleSelect2 = clazz(AbstractSelect2, {
        ...
        // single
        enableInterface(...args) {
            if (this.parent.enableInterface.apply(this, args)) {
        ...
        opening(...args) {
            ...
            this.parent.opening.apply(this, args);
        ...
        close(...args) {
            if (!this.opened()) return;
            this.parent.close.apply(this, args);
        ...
        // single
        cancel(...args) {
            this.parent.cancel.apply(this, args);
            this.focusser.prop("disabled", false);
        ...
        destroy(...args) {
            ...
            this.parent.destroy.apply(this, args);
        },
        ...
        // single
        prepareOpts(...args) {
            var opts = this.parent.prepareOpts.apply(this, args);
            var self=this;
            ...
            if (opts.element.get(0).tagName.toLowerCase() === "select") {
                ...
                opts.initSelection = opts.initSelection || ((element, callback) => {
                    var id = element.val();
        ...
        getPlaceholder(...args) {
            ...
            return this.parent.getPlaceholder.apply(this, args);
        },
    ...
    MultiSelect2 = clazz(AbstractSelect2, {
        ...
        prepareOpts(...args) {
            ...
            if (opts.element.get(0).tagName.toLowerCase() === "select") {
                ...
                opts.initSelection = opts.initSelection || ((element, callback) => {
                    ...
                    opts.query({
                        matcher(term, text, el) {
                            var is_match = $.grep(ids, id => equal(id, opts.id(el))).length;
                            if (is_match) {
                                matches.push(el);
                            }
                            return is_match;
        ...
        postprocessResults(data, initial, noHighlightUpdate) {
            ...
            var compound = this.results.find(".select2-result-with-children");
            var self = this;
        ...
        resizeSearch() {
            ...
            var containerLeft;
            var searchWidth;
            var sideBorderPadding = getSideBorderPadding(this.search);
    ...
    $.fn.select2 = function () {
        ...
        var value;
        var multiple;
        var allowedMethods = ["val", "destroy", "opened", "open", "close", "focus", "isFocused", "container", "dropdown", "onSortStart", "onSortEnd", "enable", "disable", "readonly", "positionDropdown", "data", "search"];
        var valueMethods = ["opened", "isFocused", "container", "dropdown"];
        var propertyMethods = ["val", "data"];
        var methodsMap = { search: "externalSearch" };
    ...
    $.fn.select2.defaults = {
        ...
        formatMatches(matches) { return matches + " results are available, use up and down arrow keys to navigate."; },
        formatNoMatches() { return "No matches found"; },
        formatInputTooShort(input, min) { var n = min - input.length; return "Please enter " + n + " or more character" + (n == 1? "" : "s"); },
        formatInputTooLong(input, max) { var n = input.length - max; return "Please delete " + n + " character" + (n == 1? "" : "s"); },
        ...
        hideSelectionFromResult(selectedObject) { return undefined; },
