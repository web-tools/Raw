...
*/
CKEDITOR.dialog.add("scaytcheck",j => {
 ...
 function p(a){for(var a=(""+a).split(","),b=0,e=a.length;b<e;b+=1)f.getById(a[b]).$.style.display="inline"}function q(a){for(var a=(""+a).split(","),b=0,e=a.length;b<e;b+=1)f.getById(a[b]).$.style.display="none"}function r(a){f.getById("dic_name_"+b).$.value=a}
 var s=!0;
 var h;
 var f=CKEDITOR.document;
 var b=j.name;
 var l=CKEDITOR.plugins.scayt.getUiTabs(j);
 var g;
 var t=[];
 var u=0;
 var m=["dic_create_"+b+",dic_restore_"+b,"dic_rename_"+b+",dic_delete_"+b];
 var v=["mixedCase","mixedWithDigits","allCaps","ignoreDomainNames"];
 g=j.lang.scayt;
 ...
 label:g.aboutTab,elements:[{type:"html",id:"about",style:"margin: 5px 5px;",html:'<div><div id="scayt_about_'+b+'"></div></div>'}]}];

 var B={title:g.title,minWidth:360,minHeight:220,onShow() {var a=this;a.data=j.fire("scaytDialog",{});a.options=a.data.scayt_control.option();a.chosed_lang=a.sLang=a.data.scayt_control.sLang;if(!a.data||!a.data.scayt||!a.data.scayt_control)alert("Error loading application service"),a.hide();else{var b=0;s?a.data.scayt.getCaption(j.langCode||"en",e => {0<b++||
 (h=e,A.apply(a),y.apply(a),s=!1)}):y.apply(a);a.selectPage(a.data.tab)}},onOk() {var a=this.data.scayt_control;a.option(this.options);a.setLang(this.chosed_lang);a.refresh()},onCancel() {var a=w(),f;for(f in a)a[f].checked=!1;a="undefined"!=typeof document.forms["languagesbar_"+b]?document.forms["languagesbar_"+b].scayt_lang:[];x(a,"")},contents:t};

 CKEDITOR.plugins.scayt.getScayt(j);for(g=0;g<l.length;g++)1==l[g]&&(t[t.length]=z[g]);1==l[2]&&(u=1);

 var A=function(){function a(a){var c=
 ...
 e.ltr);i.sort((a, b) => b.lang>a.lang?-1:1);e=f.getById("scayt_lcol_"+b);d=f.getById("scayt_rcol_"+b);for(c=0;c<i.length;c++)(c<i.length/2?e:d).append(i[c].radio)}var j={dic_create(a, b, c) {var d=c[0]+","+c[1],e=h.err_dic_create,f=h.succ_dic_create;window.scayt.createUserDictionary(b,a => {q(d);p(c[1]);f=f.replace("%s",a.dname);o(f)},a => {e=e.replace("%s",a.dname);n(e+"( "+(a.message||"")+")")})},dic_rename(a, b) {var c=h.err_dic_rename||"",d=h.succ_dic_rename||
 "";window.scayt.renameUserDictionary(b,a => {d=d.replace("%s",a.dname);r(b);o(d)},a => {c=c.replace("%s",a.dname);r(b);n(c+"( "+(a.message||"")+" )")})},dic_delete(a, b, c) {var d=c[0]+","+c[1],e=h.err_dic_delete,f=h.succ_dic_delete;window.scayt.deleteUserDictionary(a => {f=f.replace("%s",a.dname);q(d);p(c[0]);r("");o(f)},a => {e=e.replace("%s",a.dname);n(e)})}};j.dic_restore=k.dic_restore||((a, b, c) => {var d=c[0]+","+c[1],e=h.err_dic_restore,f=h.succ_dic_restore;
 window.scayt.restoreUserDictionary(b,a => {f=f.replace("%s",a.dname);q(d);p(c[1]);o(f)},a => {e=e.replace("%s",a.dname);n(e)})});i=(m[0]+","+m[1]).split(",");c=0;for(e=i.length;c<e;c+=1)if(d=f.getById(i[c]))d.on("click",a,this)};

 var y=function(){var a=this;if(1==l[0])for(var g=w(),e=0,d=g.length;e<d;e++){var h=g[e].id,i=f.getById(h);if(i&&(g[e].checked=!1,1==a.options[h.split("_")[0]]&&(g[e].checked=!0),s))i.on("click",function(){a.options[this.getId().split("_")[0]]=this.$.checked?1:0})}1==
 l[1]&&(g=f.getById("cke_option"+a.sLang),x(g.$,a.sLang));u&&(window.scayt.getNameUserDictionary(a => {a=a.dname;q(m[0]+","+m[1]);if(a){f.getById("dic_name_"+b).setValue(a);p(m[1])}else p(m[0])},() => {f.getById("dic_name_"+b).setValue("")}),o(""))};
